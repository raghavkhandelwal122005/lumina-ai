...
export default function LogsPage() {
    ...
    // Compute a basic health risk score from the logged vitals (0–100 scale)
    const computeRiskScore = (hrVal: string, bpSysVal: string, bpDiaVal: string, sugarVal: string): string => {
        let score = 10; // base healthy score
        const heartRate = parseInt(hrVal) || 0;
        const bpS = parseInt(bpSysVal) || 0;
        const bpD = parseInt(bpDiaVal) || 0;
        const sugar = parseInt(sugarVal) || 0;

        // Heart rate risk
        if (heartRate > 100) score += 25;
        else if (heartRate > 90) score += 15;
        else if (heartRate < 50 && heartRate > 0) score += 20;

        // Blood pressure risk
        if (bpS > 140 || bpD > 90) score += 25;
        else if (bpS > 120 || bpD > 80) score += 12;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            ...
            sugar_level: logType === 'Vitals' ? sugar : null,
                            ...
                            <div className="flex-1">
                                ...
                                {log.vitals && log.type === 'Vitals' && (
                                    <div className="flex flex-wrap gap-2 mt-3">
                                        ...
                                        {(log as any).vitals.sugarLevel && (
                                            <span className="inline-flex items-center gap-1 text-xs font-bold bg-slate-100 text-slate-600 px-3 py-1 rounded-full"><Thermometer size={12} /> Sugar: {(log as any).vitals.sugarLevel}</span>
                                        )}
                                        {(log as any).vitals.weight && (
                                            <span className="inline-flex items-center gap-1 text-xs font-bold bg-slate-100 text-slate-600 px-3 py-1 rounded-full"><Zap size={12} /> Weight: {(log as any).vitals.weight}kg</span>
                                        )}
                                    </div>
                                )}
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            ...
                            {logType === 'Vitals' && (
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-2">Sugar Level (mg/dL)</label>
                                        <input type="number" value={sugar} onChange={(e) => setSugar(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200 focus:border-[#25418F] focus:outline-none focus:ring-1 focus:ring-[#25418F]" placeholder="95" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-2">Weight (kg)</label>
                                        <input type="number" value={weight} onChange={(e) => setWeight(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200 focus:border-[#25418F] focus:outline-none focus:ring-1 focus:ring-[#25418F]" placeholder="70" />
                                    </div>
                                </div>
                            )}
